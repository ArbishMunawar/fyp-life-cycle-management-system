...
// adminSlice.js — add these two thunks
...
export const bulkImportStudents = createAsyncThunk(
  ...
  async (file, thunkAPI) => {
    try {
      ...
      toast.success(res.data.message);
      return res.data.data; // { created: [...], failed: [...] }
    } catch (error) {
...
export const bulkImportTeachers = createAsyncThunk(
  "bulkImportTeachers",
  async (file, thunkAPI) => {
    try {
      const formData = new FormData();
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(bulkImportStudents.fulfilled, (state, action) => {
        // Add newly created users to the users list
        const newUsers = action.payload?.created || [];
        state.users = [...newUsers, ...state.users];
      })
      .addCase(bulkImportTeachers.fulfilled, (state, action) => {
        const newUsers = action.payload?.created || [];
        state.users = [...newUsers, ...state.users];
      });
  },
