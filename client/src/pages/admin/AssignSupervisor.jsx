...
const AssignSupervisor = () => {
  ...
  const res = await dispatch(
    assignSupervisor({ studentId, supervisorId }),  // ✅ changed
  );
  setPendingFor(null);

  if (assignSupervisor.fulfilled.match(res)) {  // ✅ changed
                ...
                <tr>
                  {headers.map((h) => {
                    return (
                      <th
                        key={h}
                        className="px-6 py-3 text-left text-xs font-medium text-slate-500 upper tracking-wider"
                      >
                        {h}
                      </th>
                    );
