...
const AdminDashboard = () => {
  ...
  const handleDownload = async (projectId, fileId, name) => {
    try {
      const action = await dispatch(downloadProjectFile({ projectId, fileId }));

      if (downloadProjectFile.fulfilled.match(action)) {
  ...
  // console.log("Sliced lates Notifications for Render:", latestNotifications);
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card">
                  ...
                  <div key={n._id} className="flex items-center text-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <span
                          className={`px-2 py-0.5 rounded text-sm font-medium capitalize ${getBadgeClasses("type", n.type)}`}
                        >
                          {n.type}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded text-sm font-medium capitalize ${getBulletColor(
                            n.type,
                            n.priority,
                          )}`}
                        >
            ...
            <div className="space-y-3">
              {latestNotifications.map((n) => {
                // 💡 Safe fallbacks in case your backend uses different property names
                // const displayMessage =
                //   n.message || n.text || n.title || "New Notification";
                // const displayType = n.type || n.notificationType || "system";
                // const displayPriority = n.priority || "low";
                // Around line 180 in your AdminDashboard.jsx, ensure your message extractor looks like this:
                const displayMessage = n.message || "Activity updated";
                const displayType = n.type || "system";
                const displayPriority = n.priority || "low";

                return (
                  <div
                    key={n._id || Math.random()}
                    className="flex items-center text-sm p-2 hover:bg-slate-50 rounded"
                  >
                    <div
                      className={`mt-1 w-2 h-2 ${getBulletColor(displayType, displayPriority)} rounded-full mr-3 shrink-0`}
                    />
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${getBadgeClasses("type", displayType)}`}
                        >
                          {displayType}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${getBulletColor(
                            displayType,
                            displayPriority,
                          )} text-white`}
                        >
