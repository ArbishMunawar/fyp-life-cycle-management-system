...
const TeacherFiles = () => {
  ...
  const normalizeFile = (f) => {
    ...
    if (["pdf", "doc", "docx", "txt"].includes(type)) {
      ...
      category = "code";
    } else if (["jpeg", "png", "jpg", "avif", "gif"].includes(type)) {
      category = "image";
    }
  ...
  //     return;
  //   }
  //   const res = await dispatch(downloadTeacherFile({ projectId:file.projectId, fileId:file.id })).then(
  //     (res) => {
  ...
  try {
    const res = await dispatch(
      downloadTeacherFile({ projectId, fileId })
    );

    if (downloadTeacherFile.rejected.match(res)) {
      toast.error(res.payload || "Download failed");
      return;
    }
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex justify-between items-center">
              <h1 className="card-title"> Download Files</h1>
        ...
        {viewMode === "grid" ? (
          ...
          /* LIST / DATA TABLE INTERFACE LAYOUT COMPONENT */
          <div className="card overflow-x-auto rounded-xl border bg-white shadow-sm">
            <table className="min-w-full border border-slate-200 font-semibold text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-700 border-b">
                  {tableHeadData.map((t) => (
                    <th
                      key={t}
                      className="py-3 px-4 text-left font-semibold text-sm text-slate-600"
                    >
                      {t}
                    </th>
                  ))}
                ...
                {filteredFiles.map((file) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <button
                        onClick={() => handleDownloadFile(file)}
                        className="btn-primary btn-small px-3 py-1.5 rounded bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium transition-all duration-200"
                      >
                        Download
                      </button>
                    </td>
